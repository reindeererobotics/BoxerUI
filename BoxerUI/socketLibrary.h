...
struct initialize socket_initialise(int ipType, int port, char* ip) {
    ...
    setup.sockets.sin_addr.s_addr = inet_addr(ip); //set the address your using or sending to
...
int listen_socket(int a, int b, char* c) {
...
int connect_socket(int a, int b, char* c) {
    struct initialize setup = socket_initialise(a, b, c); //iptype port address
    int network_socket = setup.network_socket;
    struct sockaddr_in server_address = setup.sockets;
